...
def signup():
    ...
    if request.method == "POST":
        ...
        if not user_check:
            ...
            error_message = "入力されたユーザIDはすでに使用されています"
...
def login():
    ...
    if request.method == "POST":
        ...
        #ログインチェック
        db = get_db()
        user_data = db.execute("select password from user where userid = ?",[userid,]).fetchone()
        ...
        if user_data is not None:
            if check_password_hash(user_data[0], password):
                ...
                login_user(user)
                return redirect("/")
        error_message = "入力されたユーザIDもしくはパスワードが間違っています"
